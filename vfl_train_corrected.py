"""
VFL Split Learning Trainer - Corrected for proper evaluation on full validation sets
"""
...
def load_vfl_data(csv_path: Path, label_col: str, normal_label: str, test_size: float, val_size: float, random_state: int):
    """Load and prepare data for VFL - returns FULL sets for training and evaluation"""
    ...
    # Extract normal samples for training ONLY
    ...
    # Scale ALL data (including anomalies in val/test)
    X_train_normal_scaled = scaler.transform(X_train_normal).astype(np.float32)
    X_val_scaled = scaler.transform(X_val).astype(np.float32)  # Full validation set
    X_test_scaled = scaler.transform(X_test).astype(np.float32)  # Full test set
    
    return {
        "X_train_normal": X_train_normal_scaled,  # Only normal samples for training
        "X_val": X_val_scaled,  # Full validation set for evaluation
        "y_val": y_val,
...
# VFL STRATEGY 1: SEMANTIC GROUPING (3 CLIENTS) - Corrected features
VFL_FEATURE_SETS = {
...
def vfl_train_round(client_models: Dict[str, nn.Module], global_model: nn.Module, 
    ...
    for batch_start in range(0, n_samples, batch_size):
        ...
        x_fwd = torch.from_numpy(train_data["client_0_forward_metrics"][batch_start:batch_end]).to(device)
        x_bwd = torch.from_numpy(train_data["client_1_backward_metrics"][batch_start:batch_end]).to(device)
        x_flow = torch.from_numpy(train_data["client_2_flow_and_flags"][batch_start:batch_end]).to(device)
        x_orig = torch.from_numpy(train_x[batch_start:batch_end]).to(device)
        ...
        loss = criterion(reconstructed, x_orig)
...
def evaluate_vfl(client_models: Dict[str, nn.Module], global_model: nn.Module,
    ...
    """Evaluate VFL model on full validation set"""
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="vfl_artifacts_semantic")
    ...
    val_data = partition_features(data["X_val"], data["feature_names"])
    ...
    print(f"\nVFL Architecture (Strategy 1 - Semantic Grouping):")
    print(f"  Clients: {len(client_models)}")
    print(f"  Client 0 (Forward): 13 features")
    print(f"  Client 1 (Backward): 11 features")
    print(f"  Client 2 (Flow/Flags): 28 features")
    print(f"  Embedding dimension per client: {args.embedding_dim}")
    print(f"  Total aggregated embedding: {total_embedding_dim}")
    print(f"  Output dimension: {output_dim}")
    print(f"  Training samples (normal only): {len(data['X_train_normal'])}")
    print(f"  Validation samples (all): {len(data['X_val'])}")
    ...
    for round_num in range(1, args.rounds + 1):
        print(f"Round {round_num:2d}/{args.rounds} | ", end="", flush=True)
        
        loss = vfl_train_round(client_models, global_model, data["X_train_normal"], 
                               train_data, device, args.batch_size, args.lr)
        
        metrics = evaluate_vfl(client_models, global_model, data["X_val"],
    ...
    if best_state:
        ...
        print(f"\n✓ Best model from round {best_state['round']}")
    ...
    print(f"  ✓ {global_model_path.name}")
    ...
    print(f"\nBest Metrics (Round {best_state['round']}):")
